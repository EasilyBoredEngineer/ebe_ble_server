...
void EBEBleServer::setup_characteristics() {
  this->my_characteristic_ = this->service_->create_characteristic(
...
void EBEBleServer::process_incoming_data_() {
  ...
  if (incoming_data_.size() <= sizeof(MyData)) {
    ...
    ESP_LOGD(TAG, "float one is: %f, float two is %f, float 3 is %f, float 4 is %f", data_.float1_, data_.float2_,
             data_.float3_, data_.float4_);
  } else {
...
void EBEBleServer::on_client_disconnect() { ESP_LOGD(TAG, "Emulator - BLE Client disconnected"); };

void EBEBleServer::on_client_connect() { ESP_LOGD(TAG, "Emulator - BLE Client connected"); };
